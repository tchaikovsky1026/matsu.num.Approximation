...
final class NewtonPolynomialTest {
    ...
    public static class 生成の例外テスト {
        ...
        public void test_サイズ0は受け入れられない() {
            NewtonPolynomial.from(PROVIDER.createArray(0), PROVIDER.createArray(0), PROVIDER);
        }

        @Test(expected = IllegalArgumentException.class)
        public void test_サイズが異なると受け入れられない() {
            NewtonPolynomial.from(PROVIDER.createArray(1), PROVIDER.createArray(0), PROVIDER);
        }
    }
